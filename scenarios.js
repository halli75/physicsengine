class SimultaneityScenario {
	constructor() {
		...
		station.loadModel('assets/objects/railway_station.obj', 0.4, 0.4, 0.4);
		...
		train.loadModel('assets/objects/train.obj', 0.2, 0.2, 0.2);
		train.createBuffers();
		train.vel = (t) => {
			return [vel,0,0];
		}
...
class LadderParadoxScenario {
	constructor() {
		...
		const ladder = new Body([], ladderTex);
		...
		floor.loadModel('assets/objects/podloga.obj', 0.5,0.5,0.5);
		floor.createBuffers();
		floor.rotation = (t) => {
			return m4.rotationY(Math.PI/2);
		}
...
class TwinParadoxScenario {
	constructor() {
		...
		//transformation_method = PAST_LIGHTCONE;
		transformation_method = PRESENT_HYPERSURFACE;
		sky_color = [0,0,0];
		
		const earth = new Body([-10, 6, 0], earthTex);
		earth.loadModel('assets/objects/earth.obj', 2, 2, 2);
		earth.createBuffers();
	    earth.rotation = (t) => {
		...
		rocket.loadModel('assets/objects/rocket.obj', 2,2,2);
...
class Gate {
	constructor() {
		...
		const ladder = new Body([0, 1, 0], stationTex);
		//ladder.loadModel('assets/objects/untitled.obj', 1, 1, 1);
		ladder.loadModel('assets/objects/b2.obj', 1, 1, 1);
		ladder.createBuffers();
